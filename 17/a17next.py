...
with open("17/input.txt", "r") as input:
    ...
    while len(fifo) > 0:
        ...
        for next_pos, next_dir in [(move_pos(pos, dir2), dir2) for i, dir2 in enumerate(POS_NEXT[dir])]:
            ...
            if next_linear_repeat == 10:
                continue
            if next_dir != dir and linear_repeat < 3 and dir != '*':
                continue
    ...
    end_entry = map[the_end[1]][the_end[0]]
    print("least energy {} last_pos contents {}".format(min(list(end_entry.values())), end_entry))
