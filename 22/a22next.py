...
def chain_reaction():
    ...
    for b in bricks:
        in_chain = { b }
        sub_bricks = { b }
        while len(sub_bricks) > 0:
            next_bricks = set()
            for sb in sub_bricks:
                more_bricks = [a for a in sb.aboves if (a.belows <= in_chain)]
                in_chain.update(more_bricks)
                next_bricks.update(more_bricks)
            sub_bricks = next_bricks
        all_reactions[b] = len(in_chain) - 1
        print("{} has chain-reaction of {}".format(b, len(in_chain) -1))
...
print("sum of all chain reactions: {}".format(sum(chain_reaction().values())))
